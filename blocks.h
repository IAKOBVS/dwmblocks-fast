...
/* { .func = b_write_shell, .arg = "some_arg | other_arg ", .pad_left = "my arg:", .pad_right = " | ", .interval = 0, .signal = SIG_AUDIO }, */
#	endif
...
#	endif
	...
	/* Disk */
	{ .func = b_write_disk_usage_percent,  .arg = "/home",       .pad_left = "📁 /home ", .pad_right = "% ",   .interval = 32,   .signal = 0          },
	{ .func = b_write_disk_usage_free,     .arg = "/home",       .pad_left = "",          .pad_right = " | ",  .interval = 32,   .signal = 0          },
	{ .func = b_write_disk_usage_percent,  .arg = "/",           .pad_left = "📁 / ",     .pad_right = "% ",   .interval = 32,   .signal = 0          },
	{ .func = b_write_disk_usage_free,     .arg = "/",           .pad_left = "",          .pad_right = " | ",  .interval = 32,   .signal = 0          },

/* Ram */
#	ifdef HAVE_PROCFS
	{ .func = b_write_ram_usage_percent,   .arg = NULL,          .pad_left = "🧠 ",       .pad_right = "% ",   .interval = 8,   .signal = 0          },
	{ .func = b_write_ram_usage_available, .arg = NULL,          .pad_left = "",          .pad_right = " | ",  .interval = 8,   .signal = 0          },
#	endif
...
/* format: [temp] [usage] */
#		ifdef HAVE_SYSFS
	{ .func = b_write_cpu_temp,            .arg = "/tmp/cfan/temp_cpu", .pad_left = "💻 ",       .pad_right = "° ",   .interval = 2,    .signal = 0          },
#		endif
...
#	if defined USE_CUDA
	...
	{ .func = b_write_gpu_temp,            .arg = "/tmp/nvspeed/temp",          .pad_left = "🚀 ",       .pad_right = "° | ",   .interval = 2,    .signal = 0          },
