...
b_read_cpu_usage_power(void)
{
	if (unlikely(fd_cpu_usage_power == -1)) {
		fd_cpu_usage_power = open("/sys/class/powercap/intel-rapl/intel-rapl:0/energy_uj", O_RDONLY);
		if (fd_cpu_usage_power < 0) {
			fd_cpu_usage_power = -2;
			return 0;
		}
		const int fd_range = open("/sys/class/powercap/intel-rapl/intel-rapl:0/max_energy_range_uj", O_RDONLY);
		if (fd_range < 0) {
